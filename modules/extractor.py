...
def handle_advanced_protections(temp_dir: str):
    ...
    if not check_dependency("d2j-dex2jar") or not check_dependency("jadx") or not check_dependency("baksmali"):
        return

    dex_files = [os.path.join(temp_dir, f) for f in os.listdir(temp_dir) if f.endswith('.dex')]
...
def extract_native_libraries(output_dir: str):
    ...
    for so_file in so_files:
        ...
        try:
            subprocess.run(["objdump", "-d", so_file, "-M", "intel", "-o", disasm_file], check=True)
            ...
            if file.endswith(".smali") or file.endswith(".xml"):
                ...
                for match in base64_matches:
                    try:
                        ...
                        logging.info(f"🔹 Found Base64 encoded string: {match} -> {decoded}")
                    except Exception:
