            ...
            if key == curses.KEY_UP and cursor_y > 0:
                ...
                try:
                    with open(manifest_path, "w", encoding="utf-8") as f:
                        f.writelines(lines)
                    stdscr.addstr(max_y - 1, 0, "✅ Changes saved! Press any key to continue.")
                    stdscr.refresh()
                    stdscr.getch()
                except Exception as e:
                    stdscr.addstr(max_y - 1, 0, f"❌ Save error: {e}", curses.color_pair(1))
                    stdscr.refresh()
                    stdscr.getch()
            elif key == ord('q'):
                break
        ...
        try:
            if file_path.endswith('.apk'):
                ...
                if not os.path.exists(manifest_path):
                    raise FileNotFoundError("AndroidManifest.xml not found in the given folder.")
